    ...
    public function onPluginsInitialized()
    {
        ...
        if (!file_exists($this->check)) {
            ...
            if (!$this->problemChecker()) {
                ...
                foreach (
                    new \GlobIterator(CACHE_DIR . $validated_prefix . '*')
                    as $fileInfo
                ) {
    ...
    protected function problemChecker()
    {
        ...
        if (strpos(php_sapi_name(), 'apache') !== false) {
            ...
            foreach ($require_apache_modules as $module) {
                if (in_array($module, $apache_modules)) {
                    ...
                    $problems_found = true;
                    $apache_module_adjective =
                        ' Apache module is not installed or enabled';
                    $apache_module_status = 'error';
                }
                $apache_status[$module] = [
                    $apache_module_status => $apache_module_adjective,
                ];
            }
        ...
        $this->results['php'] = [
            $php_version_status =>
                ...
                phpversion() .
                ') is ' .
                $php_version_adjective .
                ' than the minimum required: <b>' .
                $min_php_version .
                '</b>  - <a href="http://getgrav.org/blog/changing-php-requirements-to-5.5">Additional Information</a>',
        ];
        ...
        $this->results['gd'] = [
            $gd_status =>
                'PHP GD (Image Manipulation Library) is ' .
                $gd_adjective .
                'installed',
        ];
        ...
        $this->results['curl'] = [
            $curl_status =>
                'PHP Curl (Data Transfer Library) is ' .
                $curl_adjective .
                'installed',
        ];
        ...
        $this->results['ssl'] = [
            $ssl_status =>
                'PHP OpenSSL (Secure Sockets Library) is ' .
                $ssl_adjective .
                'installed',
        ];
        ...
        $this->results['xml'] = [
            $xml_status => 'PHP XML Library is ' . $xml_adjective . 'installed',
        ];
        ...
        $this->results['mbstring'] = [
            $mbstring_status =>
                'PHP Mbstring (Multibyte String Library) is ' .
                $mbstring_adjective .
                'installed',
        ];
        ...
        if ($this->grav['config']->get('system.media.auto_metadata_exif')) {
            ...
            $this->results['exif'] = [
                $exif_status =>
                    'PHP Exif (Exchangeable Image File Format) is ' .
                    $exif_adjective .
                    'installed',
            ];
        }
        ...
        $this->results['zip'] = [
            $zip_status =>
                'PHP Zip extension is ' . $zip_adjective . 'installed',
        ];
            ...
            if (!file_exists($file_path)) {
                ...
                if ($check_writable) {
                    if (!$is_writeable) {
                        ...
                        $problems_found = true;
                        $file_adjective .=
                            ' but is <b class="underline">not writeable</b>';
                    } else {
                        $file_adjective .=
                            ' and <b class="underline">is writeable</b>';
                    }
                }
            }
