...
class SimplesearchPlugin extends Plugin
{
    ...
    public function onPagesInitialized()
    {
        ...
        $this->query = array_filter(
            array_filter(explode(',', $query), 'trim'),
            'strlen'
        );
        ...
        if (
            ...
            foreach ($filters as $key => $filter) {
                ...
                if ($key === '@self' || $key === 'self@') {
                    ...
                    $taxonomies =
                        $filter === false
                            ? false
                            : array_merge($taxonomies, (array) $filter);
                } else {
            ...
            if ($new_approach) {
                ...
                $params['query'] = $this->config->get(
                    'plugins.simplesearch.query'
                );
                ...
                $this->collection->append(
                    $taxonomy_map
                        ->findTaxonomy($find_taxonomy, $operator)
                        ->toArray()
                );
            }
        }
        ...
        if (!isset($page->header()->simplesearch)) {
            ...
            $template_override = $this->config->get(
                ...
                foreach ($rules as $rule => $value) {
                    if (is_array($value)) {
                        foreach ($value as $nested_rule => $nested_value) {
                            if (
                                $user->authorize($rule . '.' . $nested_rule) ==
                                $nested_value
                            ) {
                                $returnCollection[$page->path()] = [
                                    'slug' => $page->slug(),
                                ];
                                break;
                            }
    ...
    public function onTwigSiteVariables()
    {
        ...
        if ($this->config->get('plugins.simplesearch.built_in_css')) {
            $this->grav['assets']->add(
                'plugin://simplesearch/css/simplesearch.css'
            );
        }

        $this->grav['assets']->addJs(
            'plugin://simplesearch/js/simplesearch.js',
            ['group' => 'bottom']
        );
    }
    ...
    private function matchText($haystack, $needle)
    {
        if (
            ...
            try {
                $result = mb_stripos(
                    iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $haystack),
                    iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $needle)
                );
            } catch (\Exception $e) {
    ...
    private function notFound($query, $page, $taxonomies)
    {
        ...
        $results = true;
        $search_content = $this->config->get(
        ...
        foreach ($searchable_types as $type) {
            if ($type === 'title') {
                $result =
                    $this->matchText(strip_tags($page->title()), $query) ===
                    false;
                ...
                $result =
                    $this->matchText(strip_tags($content), $query) === false;
            }
