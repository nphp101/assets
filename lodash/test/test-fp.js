(function () {
  ...
  (function () {
    ...
    _.each(methodNames, function (methodName) {
      ...
      QUnit.test(
        "fp." + methodName + "` should have a `placeholder` property",
        function (assert) {
          assert.expect(2);
  ...
  _.each(
    ["assignAllWith", "assignInAllWith", "extendAllWith", "mergeAllWith"],
    function (methodName) {
      var func = fp[methodName];
      ...
      QUnit.test(
        "`fp." + methodName + "` should work with more than two sources",
        function (assert) {
          assert.expect(2);
  ...
  _.each(["curry", "curryRight"], function (methodName) {
    ...
    QUnit.test(
      "fp." + methodName + "` should only accept a `func` param",
      function (assert) {
        assert.expect(1);
  ...
  _.each(["curryN", "curryRightN"], function (methodName) {
    ...
    QUnit.test(
      "fp." + methodName + "` should accept an `arity` param",
      function (assert) {
        assert.expect(1);
  ...
  (function () {
    QUnit.test("should provide the correct `customizer` arguments", function (
      ...
      var args,
        ...
        objects = [{ a: 1 }, { a: 2 }],
        stack = {
          __data__: { __data__: [objects, objects.slice().reverse()], size: 2 },
          size: 2,
        },
  ...
  (function () {
    QUnit.test("should return a iteratee with capped params", function (
      assert
    ) {
      assert.expect(1);
  ...
  _.each(["partial", "partialRight"], function (methodName) {
    ...
    QUnit.test(
      "fp." + methodName + "` should accept an `args` param",
      function (assert) {
        assert.expect(1);
    ...
    QUnit.test("fp." + methodName + "` should convert by name", function (
      assert
    ) {
      assert.expect(2);
  ...
  _.each(["range", "rangeRight"], function (methodName) {
    ...
    QUnit.test(
      "fp." +
        methodName +
        "` should have an argument order of `start` then `end`",
      function (assert) {
        assert.expect(1);
  ...
  _.each(["rangeStep", "rangeStepRight"], function (methodName) {
    ...
    QUnit.test(
      "fp." +
        methodName +
        "` should have an argument order of `step`, `start`, then `end`",
      function (assert) {
        assert.expect(1);
  ...
  (function () {
    QUnit.test("should zip together two arrays", function (assert) {
      ...
      assert.deepEqual(fp.zip([1, 2])([3, 4]), [
        [1, 3],
        [2, 4],
      ]);
    });
