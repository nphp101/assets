(function (QUnit) {
  ...
  QUnit.test("chain", function (assert) {
    ...
    assert.deepEqual(
      model.chain().pick("a", "b", "c").values().compact().value(),
      [1, 2]
    );
  });
  ...
  QUnit.test(
    "Nested change events don't clobber previous attributes",
    function (assert) {
      assert.expect(4);
      new Backbone.Model()
  ...
  QUnit.test("#1791 - `attributes` is available for `parse`", function (
    assert
  ) {
    var Model = Backbone.Model.extend({
      parse: function () {
