...
QUnit.test(
  "should throw console warning about unexpected type of customClass",
  (assert) => {
    ...
    assert.ok(
      spy.calledWith(
        'SweetAlert2: Invalid type of customClass.title! Expected string or iterable object, got "object"'
      )
    );
    assert.ok(
      spy.calledWith(
        'SweetAlert2: Invalid type of customClass.popup! Expected string or iterable object, got "number"'
      )
    );
  }
);
