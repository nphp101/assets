    ...
    SwalWithoutAnimation.fire({
      ...
      onOpen: () => {
        setTimeout(() => {
          assert.equal(
            Swal.getInput().value,
            input === "number" ? parseFloat(value) : value
          );
...
QUnit.test(
  "should throw console error when inputValue as a Promise rejects",
  (assert) => {
    ...
    SwalWithoutAnimation.fire({
      ...
      onOpen: () => {
        setTimeout(() => {
          ...
          assert.ok(
            spy.calledWith(
              "SweetAlert2: Error in inputValue promise: Error: input promise rejected"
            )
          );
          done();
        }, TIMEOUT);
...
QUnit.test(
  "should throw console warning about unexpected type of inputValue",
  (assert) => {
    const _consoleWarn = console.warn;
    const spy = sinon.spy(console, "warn");
