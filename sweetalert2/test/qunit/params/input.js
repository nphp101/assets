...
QUnit.test(
  "should throw console error about unexpected input type",
  (assert) => {
...
QUnit.test("input email + built-in email validation", (assert) => {
  ...
  setTimeout(() => {
    ...
    assert.ok(
      Swal.getValidationMessage().textContent.indexOf(
...
QUnit.test("input url + built-in url validation", (assert) => {
  ...
  setTimeout(() => {
    assert.ok(isVisible(Swal.getValidationMessage()));
    assert.ok(
      Swal.getValidationMessage().textContent.indexOf("Invalid URL") !== -1
    );
...
QUnit.test('input type "select", inputOptions Map', (assert) => {
  ...
  assert.equal(
    $(".swal2-select option:nth-child(1)").innerHTML,
    "Richard Stallman"
  );
  assert.equal($(".swal2-select option:nth-child(1)").value, "2");
  assert.equal(
    $(".swal2-select option:nth-child(2)").innerHTML,
    "Linus Torvalds"
  );
...
QUnit.test('input type "radio", inputOptions Map', (assert) => {
  ...
  assert.equal(
    $(".swal2-radio label:nth-child(1)").textContent,
    "Richard Stallman"
  );
  assert.equal($(".swal2-radio label:nth-child(1) input").value, "2");
  assert.equal(
    $(".swal2-radio label:nth-child(2)").textContent,
    "Linus Torvalds"
  );
...
QUnit.test("input radio", (assert) => {
  ...
  assert.equal($(".swal2-radio").querySelectorAll("label").length, 2);
  assert.equal(
    $(".swal2-radio").querySelectorAll('input[type="radio"]').length,
    2
  );
});
...
QUnit.test(
  "Swal.getInput() should return null when a popup is disposed",
  (assert) => {
    const done = assert.async();
...
QUnit.test(
  "popup should expand and shrink accordingly to textarea width",
  (assert) => {
    const done = assert.async();
    SwalWithoutAnimation.fire({
      input: "textarea",
