...
class PositionChecker {
  constructor(container, offset) {
    ...
    this._containerCenter =
      container === window
        ...
        : container.offsetTop + container.clientHeight / 2;
    this._containerBottom =
      container === window
        ? window.innerHeight
        : container.offsetTop + container.clientHeight;
    this._containerLeft = container === window ? 0 : container.offsetLeft;
    this._containerMiddle =
      container === window
        ? window.innerWidth / 2
        : container.offsetLeft + container.clientWidth / 2;
    this._containerRight =
      container === window
        ? window.innerWidth
        : container.offsetLeft + container.clientWidth;
  ...
  isBottom(clientRect) {
    return (
      Math.abs(clientRect.bottom - (this._containerBottom - this._offset)) < 1
    );
  }
  ...
  isRight(clientRect) {
    return (
      Math.abs(clientRect.right - (this._containerRight - this._offset)) < 1
    );
  }
  ...
  check(pos, clientRect) {
    ...
    return (
      this._checkFunctions[verPos](clientRect) &&
      this._checkFunctions[horPos](clientRect)
    );
  }
}
...
QUnit.test("Toast positions", (assert) => {
  ...
  allowedPostions.forEach((position) => {
    ...
    assert.ok(
      checkPosition.check(position, swalRect),
      `Failed toast position: on "${navigator.userAgent}": ${position} \n Swal: (${swalRect.top}, ${swalRect.right}, ${swalRect.bottom}, ${swalRect.left})x(${swalRect.height}, ${swalRect.width})\n Window: (${window.innerHeight} ${window.innerWidth})`
    );
    Swal.close();
  });
...
QUnit.test("Modal positions with target", (assert) => {
  ...
  const dummyTargetElement = Object.assign(document.createElement("div"), {
    id: "dummy-target",
  });
  dummyTargetElement.setAttribute(
    "style",
    `width: ${targetWidth}px; height: ${targetHeight}px; position: relative;`
  );
  ...
  allowedPostions.forEach((position) => {
    ...
    assert.ok(
      checkPosition.check(position, swalRect),
      `Failed modal position with target on "${navigator.userAgent}": ${position} \n Swal: (${swalRect.top}, ${swalRect.right}, ${swalRect.bottom}, ${swalRect.left})x(${swalRect.height}, ${swalRect.width})`
    );
    Swal.close();
  });
...
QUnit.test("Toast positions with target", (assert) => {
  ...
  allowedPostions.forEach((position) => {
    ...
    assert.ok(
      checkPosition.check(position, swalRect),
      `Failed toast position with target on "${navigator.userAgent}": ${position}\n Swal: (${swalRect.top}, ${swalRect.right}, ${swalRect.bottom}, ${swalRect.left})x(${swalRect.height}, ${swalRect.width})`
    );
    Swal.close();
  });
