...
QUnit.test(".swal2-toast-shown", (assert) => {
  ...
  assert.notOk(
    document.documentElement.classList.contains("swal2-toast-shown")
  );
});

QUnit.test(
  "should throw console warnings for incompatible parameters",
  (assert) => {
    ...
    assert.ok(
      spy.calledWith(
        'SweetAlert2: The parameter "allowOutsideClick" is incompatible with toasts'
      )
    );
...
QUnit.test(
  "toast click closes when no buttons or input are specified",
  (assert) => {
...
QUnit.test(
  "toast click does not close if cancel button is present",
  (assert) => {
    const done = assert.async();
...
QUnit.test(
  "toast click does not close if input option is specified",
  (assert) => {
    const done = assert.async();
