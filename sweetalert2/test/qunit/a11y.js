...
QUnit.test(
  "should focus body in there is not previuos active element",
  (assert) => {
    const done = assert.async();

    SwalWithoutAnimation.fire(
      "I was called programmatically and will focus body after closing"
    );
    Swal.clickConfirm();
...
QUnit.test(
  'should set aria-hidden="true" to all body children if modal',
  (assert) => {
...
QUnit.test(
  'should not set aria-hidden="true" on the custom container (target)',
  (assert) => {
    const div = document.createElement("div");
    document.body.appendChild(div);
...
QUnit.test(
  'should not set aria-hidden="true" when `backdrop: false`',
  (assert) => {
    const div = document.createElement("div");
    document.body.appendChild(div);
