...
QUnit.test(
  "should not throw console error about undefined params and treat them as empty strings",
  (assert) => {
    const _consoleError = console.error;
    const spy = sinon.spy(console, "error");
...
QUnit.test(
  "should not throw console error when <svg> tags are present (#1289)",
  (assert) => {
...
QUnit.test(
  "the vertical scrollbar should be hidden and the according padding-right should be set",
  (assert) => {
    const done = assert.async();
    ...
    Swal.fire({
      title:
        "The body has visible scrollbar, I will hide it and adjust padding-right on body",
      onAfterClose: () => {
...
QUnit.test(
  "the vertical scrollbar should be restored before a toast is fired after a modal",
  (assert) => {
    const done = assert.async();
    const talltDiv = document.createElement("div");
    talltDiv.innerHTML = Array(100).join("<div>lorem ipsum</div>");
...
QUnit.test("content/title is set (html)", (assert) => {
  ...
  assert.equal(
    $(".swal2-content").querySelectorAll("p, img, button").length,
    3
  );
});
...
QUnit.test("content/title is set (text)", (assert) => {
  ...
  assert.equal(
    $(".swal2-title").innerHTML,
    "&lt;strong&gt;Strong&lt;/strong&gt;, &lt;em&gt;Emphasis&lt;/em&gt;"
  );
  assert.equal(
    $("#swal2-content").innerHTML,
...
QUnit.test("validation message", (assert) => {
  ...
  const inputValidator = (value) =>
    Promise.resolve(!value && "no falsy values");
...
QUnit.test(
  "should throw console error about unexpected type of InputOptions",
  (assert) => {
    const _consoleError = console.error;
    const spy = sinon.spy(console, "error");
    ...
    assert.ok(
      spy.calledWith(
        "SweetAlert2: Unexpected type of inputOptions! Expected object, Map or Promise, got string"
      )
    );
  }
);
...
QUnit.test("addition and removal of backdrop", (assert) => {
  ...
  assert.notOk(
    document.documentElement.classList.contains("swal2-no-backdrop")
  );
});
...
QUnit.test("visual apperarance", (assert) => {
  ...
  assert.equal(
    window.getComputedStyle(Swal.getPopup()).backgroundColor,
    "rgb(255, 0, 0)"
  );
