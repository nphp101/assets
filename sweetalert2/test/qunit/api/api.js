...
QUnit.test("properties of `Swal` class are consistent", (assert) => {
  ...
  const assertConsistent = (postfix) => {
    ...
    const extraPropNames = currentSwalPropNames.filter(
      (key) => !initialSwalPropNames.includes(key)
    );
    assert.deepEqual(
      extraPropNames.length,
      0,
      `# of extra properties ${postfix}`
    );
    assert.deepEqual(
      extraPropNames.join(","),
      "",
      `extra property names ${postfix}`
    );
    const missingProps = currentSwalPropNames.filter(
      (key) => !currentSwalPropNames.includes(key)
    );
    assert.deepEqual(
      missingProps.length,
      0,
      `# of missing properties ${postfix}`
    );
    assert.deepEqual(
      missingProps.join(","),
      "",
      `missing property names ${postfix}`
    );
  };
